...
public class CalendarManager {
    ...
    public CalendarManager(@NonNull LocalDate selected, int interval) {
        this(selected, State.WEEK, LocalDate.now().minusWeeks(interval), LocalDate.now().plusWeeks(interval), null);
    }

    public CalendarManager(@NonNull LocalDate selected, @Nullable LocalDate minDate,
                           @Nullable LocalDate maxDate, @NonNull State state) {
    ...
    public void init(@NonNull LocalDate date, @Nullable LocalDate minDate, @Nullable LocalDate maxDate) {
        mSelected = LocalDate.now();
        setActiveMonth(date);
        mMinDate = minDate.withDayOfWeek(1);
        mMaxDate = maxDate.withDayOfWeek(7);
        ...
        while (minDate.isBefore(maxDate.withDayOfWeek(7))) {
            weeks.add(new Week(minDate, mToday, mMinDate, mMaxDate));
